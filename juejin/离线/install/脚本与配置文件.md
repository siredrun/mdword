...
ODS层业务数据23张表的建表sql，进入hive选择gamll数据库执行下面语句即可。
...
DWD层建表sql2+10+14+2（时间维度临时表和用户维度临时拉链表）=28张表的建表sql，进入hive选择gamll数据库执行下面语句即可。包含的表目录如下：
...
DWS层建表sql5张表的建表sql，进入hive选择gamll数据库执行下面语句即可。包含的表目录如下：
...
进入hive选择gamll数据库执行下面语句即可。包含的表目录如下：
