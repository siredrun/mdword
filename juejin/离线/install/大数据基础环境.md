...
```
192.168.163.11 hadoop102 16g2核 100g磁盘
192.168.163.12 hadoop103 8g2核 100g磁盘
192.168.163.13 hadoop104 8g2核 100g磁盘
```
...
192.168.163.11 hadoop102 
192.168.163.12 hadoop103
192.168.163.13 hadoop104
```
...
sudo useradd admin
sudo passwd admin
id admin #查看用户名所在的用户组
sudo chown admin:admin /opt/* # 修改文件夹的所属用户和用户组
sudo chown -R admin:admin /opt/* # 件夹及所有子文件（夹）所属用户和用户组
su admin
```
...
2. 拷贝公钥到所有机器：
ssh-copy-id admin@hadoop102
ssh-copy-id admin@hadoop103
ssh-copy-id admin@hadoop104
3.测试免密登录：
ssh hadoop102
ssh hadoop103
ssh hadoop104
