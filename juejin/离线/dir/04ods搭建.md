...
# 创建输入数据是lzo输出是text，外部表示支持json解析的分区表。建输入数据是lzo输出是text，支持json解析的分区表。
...
hive (gmall)> load data inpath '/origin_data/gmall/log/topic_start/2020-05-20' into table gmall.ods_start_log partition(dt='2020-05-20');
